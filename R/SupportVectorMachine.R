#' Main function to calculate performance of Support Vector Machine model for classification
#' @description After pretreatment on dataset this function calculates performance
#' of SVM model through iterations and returns performance metrics.
...
#'
#' @examples
#' \dontrun{SVC.run(classificationParameterList)}
SVC.run <- function(classificationParameterList){
        cat('SVC.run \n')
        ...
        print(classificationParameterList$pretreatment)
        ...
        for(i in 1:classificationParameterList$numberOfIterations) {
          ...
          # Train the Support Vector Classification (SVC) model
          modelFit <- svm(sensory ~ ., data = trainSet, kernel = "radial", cost = 1)
          ...
          if (Accuracy > bestAcc) {
            ...
            confusion_matrix <- confusion_matrix(conf_matrix, platformName, outDir, "SVC" )
